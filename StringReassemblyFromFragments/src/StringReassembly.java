...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        String finalString;
        finalString = str1.substring(0, (str1.length() - overlap)) + str2;
        return finalString;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
         ...
         */
        boolean isSub = false;
        int length = strSet.size();
        Set<String> copyStrSet = strSet.newInstance();
        Set<String> secondcopyStrSet = strSet.newInstance();
        /**
         * each string within the set is checked if the element does not contain
         * a substring of str we proceed
         */
        for (int i = 0; i < length; i++) {
            String element = strSet.removeAny();
            copyStrSet.add(element);
            if (!element.contains(str)) {
                isSub = true;
            }
        }
        strSet.transferFrom(copyStrSet); //gets strSet its values back
        /**
         * now we check if the string we added has any substrings and keep them
         * out of our final set
         */
        if (isSub) {
            for (int j = 0; j < length; j++) {
                String elements = strSet.removeAny();
                if (!str.contains(elements)) {
                    secondcopyStrSet.add(elements);
                }

            }
            secondcopyStrSet.add(str);
            strSet.clear();
            strSet.transferFrom(secondcopyStrSet);
        }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> strings = new Set2<String>();
        //used to add line to set if criteria met
        while (!input.atEOS()) {
            String row = input.nextLine();
            addToSetAvoidingSubstrings(strings, row);
        }
        return strings;
    }
